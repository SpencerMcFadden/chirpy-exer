...
describe("Password Hashing", () => {
  ...
  it("should return false for the incorrect password", async () => {
    const result = await checkPasswordHash("badPassword", hash1);
  ...
  it("should return false when hash doesn't match", async () => {
    const result = await checkPasswordHash(password1, hash2);
    expect(result).toBe(false);
  });
  it("should return false for invalid hash", async () => {
    const result = await checkPasswordHash(password1, "badHash");
    expect(result).toBe(false);
  });
});
...
describe("extractTokenFromAuthHeader", () => {
  it("should extract the token from a valid header", () => {
    ...
    expect(extractTokenFromAuthHeader(header)).toBe(token);
  });
